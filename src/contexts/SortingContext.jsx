...
function SortingProvider({ children }) {
    ...
    const [sortingInnerSwapedState, setSortingInnerSwapedState] = useState({
        min:0,
        max:0
    });
    ...
    const changeDialogBoxSwapedState = (minElement, maxElement) => {
        setSortingInnerSwapedState((prev) => ({
            ...prev,
            min: minElement,
            max: maxElement,
            swapedOne: minElement
        }));
    };
    ...
    const generateUserSortingArray = (numbersString) => {
        let userInputArray = numbersString.split(',');
    
        // Convert the array of strings to an array of numbers
        userInputArray = userInputArray.map(Number);
        
        // Now you have an array of numbers
        console.log(userInputArray);
        userInputArray.length = userInputArray.length > 12 ? 12 : userInputArray.length;
        let userInputArrayObj = userInputArray.map(item => {return {value: item, state: 'idle'}})
        setSortingState((prev) => ({
            ...prev,
            array: userInputArrayObj,
            sorted: false,
            sorting: false
        }));
        console.log(sortingState)
    ...
    const bubbleSort = async () => {
        ...
        for (let i = 0; i < arr.length; i++) {
            for (let j = 0; j < arr.length - i - 1; j++) {
                ...
                if (arr[j] > arr[j + 1]) {
                    changeDialogBoxSwapedState(arr[j+1], arr[j]);
                    let temp = arr[j];
                    arr[j] = arr[j + 1];
    ...
    const insertionSort = async () => {
        ...
        for (let i = 1; i < arr.length; i++) {
            ...
            while (j > -1 && current < arr[j]) {
                changeDialogBoxSwapedState(current, arr[j]);
                arr[j + 1] = arr[j];
    ...
    const selectionSort = async () => {
        ...
        for (let i = 0; i < arr.length; i++) {
            ...
            for (let j = i + 1; j < arr.length; j++) {
                ...
                if (arr[j] < arr[min]) {
                    changeDialogBoxSwapedState(arr[j], arr[min]);
                    changeBar(min, { state: "idle" });
                    min = j;
                    changeBar(min, { state: "selected" });
                } else {
                    changeDialogBoxSwapedState(arr[min], arr[j]);
                    changeBar(j, { state: "idle" });
                }
            }
    ...
    async function mergeSortMerger(arr, start, middle, end) {
        ...
        while (i < left.length && j < right.length) {
            if (left[i] < right[j]) {
                changeDialogBoxSwapedState(left[i], right[j]);
                changeBar(k, { value: left[i], state: "selected" });
                arr[k++] = left[i++];
            } else {
                changeDialogBoxSwapedState(right[j], left[i]);
        ...
        while (i < left.length) {
            changeDialogBoxSwapedState(left[i], 0);
            changeBar(k, { value: left[i], state: "selected" });
            arr[k++] = left[i++];
            await awaitTimeout(sortingState.delay);
        }
        ...
        while (j < right.length) {
            changeDialogBoxSwapedState(right[i], 0);
            changeBar(k, { value: right[j], state: "selected" });
            arr[k++] = right[j++];
            await awaitTimeout(sortingState.delay);
        }
    ...
    const quickSortHelper = async (arr, start = 0, end = arr.length - 1) => {
        ...
        while (i <= j) {
            ...
            if (i <= j) {
                changeDialogBoxSwapedState(arr[i], arr[j]);
    ...
    return (
        <SortingContext.Provider
            value={{
                ...
                changeSortingSpeed,
                changeAlgorithm,
                sortingInnerSwapedState,
                generateUserSortingArray
            }}
        >
            {children}
        </SortingContext.Provider>
